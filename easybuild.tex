...
\easybuild{} (version 0.5) occurred in April 2012, after 2.5 years of in-house
...
release every 4 to 6 weeks, the development team enjoys a well-earned reputation
for continuous, high quality maintenance, support, and improvement. At the time of
...
package directories, next to to over 6,000 lines for tests. This provides some
...
taken into account. Note that easyconfig files only provide the information required
...
be obtained via the \easybuild{} command line.
...
the dependency resolution mechanism is enabled via the command line option
...
packages that have an extensive list of dependencies, or when re-installing software
...
log level, modules tool and module naming scheme that should be used.
...
run time. Every time \texttt{\small eb} is used, the framework will `scan' the Python 
