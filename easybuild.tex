...
\markus{Update?}\kenneth{updated}
\markus{Add ``Currently, \easybuild{} only supports x86/Power-based Linux clusters,
however, support for Cray X-series systems is under development.''?}\kenneth{done}

Under the motto ``\emph{building software with ease}", \easybuild{} aims to provide
\markus{tone down $\rightarrow$ aims to provide}\kenneth{ok}
an easy
...
making it a well suited \markus{tone down $\rightarrow$ a well suited}\kenneth{ok}
...
software-specific easyblocks and 20 generic easyblocks \markus{Update?}\kenneth{ok}, providing support for
automatically installing a wide range of software packages. Examples range
from fairly easy-to-build programs like gzip, over basic \markus{commonly used $\rightarrow$ basic/essential?}\kenneth{basic sounds good} tools like
