...
was going to require a complete rewrite, so the Lua language was choosen because
...
in Lua was fast enough to be used directly, and hence the project stuck to Lua
...
Lmod supports the notion of assigning so-called \emph{properties} to modules,
...
using the \texttt{\small add\_property} function (Lua-only). Supported module
...
so-called \emph{hook} functions, allowing sites to plug in additional functionality. 
...
stack-based, using the \texttt{\small pushenv} function (Lua-only). For environment
...
different working environments, and allows user support teams to define a default
