...
used at HPC sites to install and manage scientific software over the lifetime of an HPC system. We start
by describing traditional module tools used by many sites, then introduce the concept of module files and various module naming
schemes in common use today.  Next, we review typical
workflows for installing scientific software. Finally,
we highlight the lack of collaboration among HPC sites and the absence
of tools for dealing with the problems mentioned along the way.

\subsection{Environment modules in Common Practice}
\label{sec:env_modules_system}

For software not installed in standard system locations, a user's
\texttt{\small\$PATH} must be modified to include the installed location of a
software package.  Other environment variables may also be required:  \texttt{\small\$LD\_LIBRARY\_PATH}, paths to include directories, etc.
...
\noindent
Typical values of \emph{\texttt{\small<subcmd>}} (among many) include \texttt{\small load},
...
that users maintain control their working environment as they switch  among
versions of the same application, library, compiler, or MPI stack.  These features explain why the environment module
...
(publicly accessible) version controlled Tcl-only
...
Today, HPC systems have multiple compilers (GCC, Intel,
Clang, PGI, \ldots) available, each available in multiple versions.

In most cases (there are some exceptions for programs written in pure C), programs compiled with one version of a compiler cannot safely link to libraries compiled with another. This means that
multiple builds of system libraries (e.g., Boost) need to be provided,
one for each compiler and version.
...
version~1.7.3 of the OpenMPI library built with different compilers might be named
as follows:
{\small
\begin{alltt}
...
OpenMPI, OpenBLAS, (Sca)LAPACK and FFTW. In this approach, the
first WRF module as shown above would bundle an entire toolchain instead of a version of WRF alone, and might take the name \texttt{\small WRF/3.5-goolf-1.6.10}.
One downside of using toolchains with terse naming conventions, however, is clear: the toolchain name generally has no direct relationship to the versions of
the encapsulated packages, so users lack top-level insight into 
this information.
...
be in the order of several hundreds. Moreover, none of these approaches prevent (especially novice) shooting themselves in the
...
all conflicting modules. For example, the OpenMPI module may
...
understood by just a small fraction or even a single member of the support
teams~\cite{Jim}. On top of this, these
...
common patterns in install procedures, leading to copy-paste, hard-to-maintain packages.
...
benefit other HPC sites: 
site-specific installation policies require labor-intensive modifications. Examples include
LosF~\cite{lmodSC11} developed by the Texas Advanced Computing Center (TACC) to
leverage RPMs developed in-house (encapsulating both the software itself
and the accompanying module file), and the XSEDE Compatible Basic Cluster
(XCBC)~\cite{Fischer14}, which provides a collection of RPMs that
...
of available tools and practices available for addressing them.
...
here: we all stand to benefit from a collaborative initiatives that
leverage the
