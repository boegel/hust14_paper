...
For software not installed in standard system locations a user's path must be
modified to include the directory to a software package.  Other enviroment
...
``\emph{package}'', the user simply does:
{\small
\begin{alltt}
...
\noindent
The second major advantage is that a user can unload
``\emph{package}'' and revert their environment to before they loaded
the ``\emph{package}''.  This ability revert or ``unload'' software 
...
In the traditional implementation, the \texttt{module} command
...
\robert{How's that for finessing that thorny topic !??!}
...
additional package-specific variables, for example, convenience
...
%\remark{changes to environment can be reverted by 'unloading' the module, which further complicates things
%(more opportunities for users to shoot themselves in the foot)}
...
{\small
\begin{alltt}
    <\emph{searchdir}>/compiler/GCC/4.8.2
\end{alltt}
}
...
package like WRF compiled with a particular compiler and linked
against a particular MPI stack, for example:
{\small
\begin{alltt}
...
Without some tools, managing the building and installing the software
packages becomes a great burden on small staffs that run HPC systems.
Modulefiles must be written for each package and version.  If done
manually by different individuals it becomes difficult to maintain
consistancy.  Are the modules catagorized similarly or are the
