...
\kenneth{2nd and 3rd advantage is both unloading? difference is not clear imho}
...
\kenneth{refer to the respective websites for the different tools?}
\kenneth{maybe already include a forward reference to Lmod here? ``More recently,
Lmod has been made available, an entirely new modules tool implemented in Lua,
providing functionality that is (mostly) compatible with the Tcl-based implementations.
We discuss Lmod in detail in Section~\ref{sec:lmod}."} 
...
\kenneth{nice job :) Maybe also include something like ``Lmod on the other hand is currently
actively developed and maintained, and has an active and thriving community." to make the contrast?}
